...
// Añade este código al final de tu app.js
// =============================================

// =============================================
// APARTADOS - Variables y Funciones
// =============================================
...
async function saveApartado(event) {
    ...
    try {
        ...
        if (response.ok) {
            ...
            closeApartadoModal();
            loadApartados();
            loadProductos(); // Actualizar stock en productos
        } else {
...
async function verDetalleApartado(id) {
    try {
        ...
        let detalleHTML = `
            <strong>Cliente:</strong> ${data.cliente?.nombre} ${data.cliente?.apellidos}<br>
            <strong>Estado:</strong> ${data.estado}<br>
            <strong>Fecha:</strong> ${formatDate(data.fecha_creacion)}<br>
            <strong>Fecha Límite:</strong> ${formatDate(data.fecha_limite)}<br>
            <strong>Total:</strong> $${parseFloat(data.monto_total).toFixed(2)}<br>
        ...
        data.detalles?.forEach(d => {
            detalleHTML += `- ${d.producto?.nombre}: ${d.cantidad} x $${parseFloat(d.precio_unitario).toFixed(2)}<br>`;
        });

        if (data.pagos?.length > 0) {
            detalleHTML += '<hr><strong>Pagos:</strong><br>';
            data.pagos.forEach(p => {
                detalleHTML += `- ${formatDate(p.fecha)}: $${parseFloat(p.monto).toFixed(2)} ${p.observacion ? '(' + p.observacion + ')' : ''}<br>`;
            });
        }
...
async function openAjusteInventarioModal() {
    ...
    try {
        ...
        productoSelect.innerHTML = '<option value="">Seleccione un producto...</option>' +
            productos.map(p => `<option value="${p.id}">${p.nombre} (Stock actual: ${p.cantidad_disponible})</option>`).join('');
    } catch (error) {
        console.error('Error loading productos:', error);
    }
...
async function verMovimientos() {
    try {
        ...
        data.slice(0, 20).forEach(m => {
            texto += `${formatDate(m.fecha)} | ${m.tipo.toUpperCase()} | ${m.producto?.nombre || 'N/A'} | Cant: ${m.cantidad} | ${m.razon || ''}\n`;
        });
...
// =============================================
// MODIFICAR showSection existente
// =============================================
// Añade estos casos al switch de showSection:
// case 'apartados': loadApartados(); break;
// case 'inventario': loadInventario(); break;

// =============================================
// CSS adicional para badges (añadir a index.html en <style>)
// =============================================
/*
.badge {
    padding: 4px 12px;
