...
function loadDashboard() {
  ...
  document.getElementById("guest-login-container").style.display =
    showGuestLoginContainer ? "block" : "none";
  document.getElementById("user-details-container").style.display =
    showGuestLoginContainer ? "none" : "block";
  document.getElementById("user-avatar").style.display = showGuestLoginContainer
    ? "none"
    : "block";
  ...
  if (currentUser.foto_url) {
    const fullUrl = currentUser.foto_url.startsWith("http")
      ? currentUser.foto_url
      : `${API_BASE_URL}${currentUser.foto_url}`;
    console.log("Setting avatar with photo URL:", fullUrl);
    document.getElementById("user-avatar").innerHTML =
      `<img src="${fullUrl}" style="width: 100%; height: 100%; object-fit: cover; border-radius: 50%;" onerror="this.parentElement.textContent='${currentUser.nombre.charAt(0).toUpperCase()}'">`;
  } else {
    console.log("No foto_url, using initial");
    document.getElementById("user-avatar").textContent = currentUser.nombre
      .charAt(0)
      .toUpperCase();
  }
...
function setupRolePermissions() {
  ...
  if (rol === "Anónimo" || rol === "Cliente") {
    ...
    const allDashCards = [
      ...
      "dash-card-consultas",
      "dash-card-apartados",
      "dash-card-inventario",
      "dash-card-cotizacion",
      "dash-card-credenciales",
      "dash-card-reembolsos",
      "dash-card-estadisticas",
    ];
  ...
  if (rol === "Vendedor") {
    ...
    if (document.getElementById("nav-consultas"))
      document.getElementById("nav-consultas").style.display = "none";
    if (document.getElementById("nav-reembolsos"))
      document.getElementById("nav-reembolsos").style.display = "none";
    ...
    if (document.getElementById("nav-cotizacion"))
      document.getElementById("nav-cotizacion").style.display = "none";
    if (document.getElementById("nav-credenciales"))
      document.getElementById("nav-credenciales").style.display = "none";
  }
  ...
  if (rol === "Empleado Superior") {
    ...
    document.getElementById("nav-ventas").style.display = "block";
    if (document.getElementById("nav-consultas"))
      document.getElementById("nav-consultas").style.display = "block";
    if (document.getElementById("nav-apartados"))
      document.getElementById("nav-apartados").style.display = "block";
    if (document.getElementById("nav-inventario"))
      document.getElementById("nav-inventario").style.display = "block";
    if (document.getElementById("nav-cotizacion"))
      document.getElementById("nav-cotizacion").style.display = "block";
    if (document.getElementById("nav-credenciales"))
      document.getElementById("nav-credenciales").style.display = "none";
    if (document.getElementById("nav-reembolsos"))
      document.getElementById("nav-reembolsos").style.display = "block";
  }
  ...
  if (rol === "Encargado") {
    ...
    if (document.getElementById("nav-estadisticas"))
      document.getElementById("nav-estadisticas").style.display = "none";
    if (document.getElementById("dash-card-estadisticas"))
      document.getElementById("dash-card-estadisticas").style.display = "none";
...
async function loadDashboardStats() {
  try {
    // Load products count
    const productos = await fetch(`${API_BASE_URL}/api/productos`).then((r) =>
      r.json(),
    );
    document.getElementById("stat-productos").textContent = productos.length;

    // Load clients count
    const clientes = await fetch(`${API_BASE_URL}/api/clientes`).then((r) =>
      r.json(),
    );
    document.getElementById("stat-clientes").textContent = clientes.length;

    // Load sales count
    const ventas = await fetch(`${API_BASE_URL}/api/ventas`).then((r) =>
      r.json(),
    );
    document.getElementById("stat-ventas").textContent = ventas.length;

    // Load low stock count
    const stockBajo = await fetch(
      `${API_BASE_URL}/api/productos/stock-bajo`,
    ).then((r) => r.json());
  ...
  productsData.forEach((product) => {
    ...
    const imgUrl = product.imagen_url
      ? product.imagen_url.startsWith("http")
        ? product.imagen_url
        : `${API_BASE_URL}${product.imagen_url}`
      : "";
    card.innerHTML = `
            <img src="${imgUrl}"
                 alt="${product.nombre}"
...
function moveCarousel(direction) {
  ...
  currentCarouselPosition = Math.max(
    0,
    Math.min(currentCarouselPosition, maxPosition),
  );
...
async function loadProducts() {
  ...
  tbody.innerHTML =
    '<tr><td colspan="8" class="loading active"><div class="spinner"></div>Cargando...</td></tr>';
  ...
  try {
    ...
    if (products.length === 0) {
      tbody.innerHTML =
        '<tr><td colspan="8" style="text-align: center; padding: 40px;">No hay productos registrados</td></tr>';
      return;
    }
    ...
    tbody.innerHTML =
      '<tr><td colspan="8" style="text-align: center; padding: 40px; color: red;">Error al cargar productos</td></tr>';
  }
}
...
async function openProductModal(productId = null) {
  ...
  const nuevaCatContainer = document.getElementById(
  ...
  if (productId) {
    ...
    if (product) {
      ...
      document.getElementById("product-precio").value =
        product.precio_unitario_actual_dolares;
...
async function saveProduct(event) {
  ...
  try {
    ...
    if (response.ok) {
      ...
      const data = await response.json();
      alert(
        "Error al guardar producto: " + (data.message || "Error desconocido"),
      );
    }
  } catch (error) {
...
async function loadClients() {
  ...
  tbody.innerHTML =
    '<tr><td colspan="5" class="loading active"><div class="spinner"></div>Cargando...</td></tr>';
  ...
  try {
    ...
    if (clients.length === 0) {
      tbody.innerHTML =
        '<tr><td colspan="5" style="text-align: center; padding: 40px;">No hay clientes registrados</td></tr>';
      return;
    }
    ...
    tbody.innerHTML =
      '<tr><td colspan="5" style="text-align: center; padding: 40px; color: red;">Error al cargar clientes</td></tr>';
  }
}
...
async function loadVentas() {
  ...
  tbody.innerHTML =
    '<tr><td colspan="6" class="loading active"><div class="spinner"></div>Cargando...</td></tr>';
  ...
  try {
    ...
    if (ventas.length === 0) {
      tbody.innerHTML =
        '<tr><td colspan="6" style="text-align: center; padding: 40px;">No hay ventas registradas</td></tr>';
      return;
    }
    ...
    ventas.forEach((venta) => {
      ...
      if (Number.isNaN(total) && venta.detalles && venta.detalles.length > 0) {
        ...
        venta.detalles.forEach((d) => {
          total +=
            (parseFloat(d.precio_unitario_tipo_dolares) || 0) *
            (d.cantidad || 0);
        });
      }
    ...
    tbody.innerHTML =
      '<tr><td colspan="6" style="text-align: center; padding: 40px; color: red;">Error al cargar ventas</td></tr>';
  }
}
...
function calculateVentaTotal() {
  ...
  rows.forEach((row) => {
    const cantidad =
      parseFloat(row.querySelector(".venta-cantidad").value) || 0;
...
async function loadEmpleados() {
  ...
  try {
    ...
    if (empleados.length === 0) {
      tbody.innerHTML =
        '<tr><td colspan="5" style="text-align: center; padding: 40px;">No hay empleados registrados</td></tr>';
      return;
    }
    ...
    tbody.innerHTML =
      '<tr><td colspan="5" style="text-align: center; padding: 40px; color: red;">Error al cargar empleados</td></tr>';
  }
}
...
async function loadProveedores() {
  ...
  try {
    ...
    if (proveedores.length === 0) {
      tbody.innerHTML =
        '<tr><td colspan="5" style="text-align: center; padding: 40px;">No hay proveedores registrados</td></tr>';
      return;
    }
    ...
    tbody.innerHTML =
      '<tr><td colspan="5" style="text-align: center; padding: 40px; color: red;">Error al cargar proveedores</td></tr>';
  }
}
...
async function loadCompras() {
  ...
  try {
    ...
    if (compras.length === 0) {
      tbody.innerHTML =
        '<tr><td colspan="5" style="text-align: center; padding: 40px;">No hay compras registradas</td></tr>';
      return;
    }
    ...
    tbody.innerHTML =
      '<tr><td colspan="5" style="text-align: center; padding: 40px; color: red;">Error al cargar compras</td></tr>';
  }
}
...
function calculateCompraTotal() {
  ...
  rows.forEach((row) => {
    const cantidad =
      parseFloat(row.querySelector(".compra-cantidad").value) || 0;
...
async function deleteCompra(id) {
  if (
    !confirm(
      "¿Está seguro de eliminar esta compra? Esto revertirá el stock de los productos.",
    )
  ) {
    return;
  }
...
function openRegisterForEmployee() {
  if (
    confirm(
      "Para registrar un nuevo empleado, debe cerrar la sesión actual e ir al registro. ¿Desea continuar?",
    )
  ) {
    handleLogout();
...
async function loadEmpleados() {
  ...
  try {
    ...
    tbody.innerHTML =
      '<tr><td colspan="5" style="text-align: center; color: var(--danger);">Error al cargar empleados</td></tr>';
  }
}
...
async function loadApartados() {
  ...
  tbody.innerHTML =
    '<tr><td colspan="9" class="loading active"><div class="spinner"></div>Cargando apartados...</td></tr>';

  try {
    const estadoFilter =
      document.getElementById("filter-apartados-estado")?.value || "";
    const url = estadoFilter
      ? `${API_BASE_URL}/api/apartados?estado=${estadoFilter}`
      : `${API_BASE_URL}/api/apartados`;
    ...
    if (data.length === 0) {
      tbody.innerHTML =
        '<tr><td colspan="9" style="text-align: center; padding: 40px;">No hay apartados registrados</td></tr>';
      return;
    }
    ...
    tbody.innerHTML =
      '<tr><td colspan="9" style="text-align: center; padding: 40px; color: var(--danger);">Error al cargar apartados</td></tr>';
  }
}
...
function addApartadoRow() {
  ...
  const productOptions = apartadosProductosData
    ...
    .map(
      (p) =>
        `<option value="${p.id}" data-precio="${p.precio_unitario_actual_dolares || p.precio || 0}" data-stock="${p.cantidad_disponible}">${p.nombre} (Stock: ${p.cantidad_disponible})</option>`,
    )
    .join("");
...
function updateApartadoTotal() {
  ...
  document.querySelectorAll("#apartado-productos-body tr").forEach((row) => {
    const cantidad =
      parseFloat(row.querySelector(".apartado-cantidad").value) || 0;
...
async function saveApartado(event) {
  ...
  const abonoInicial =
    parseFloat(document.getElementById("apartado-abono").value) || 0;
  ...
  document.querySelectorAll("#apartado-productos-body tr").forEach((row) => {
    ...
    const cantidad =
      parseInt(row.querySelector(".apartado-cantidad").value) || 0;
    const precioUnitario =
      parseFloat(row.querySelector(".apartado-precio").value) || 0;
...
function openPagoApartadoModal(apartadoId, pendiente) {
  ...
  document.getElementById("pago-apartado-pendiente").value =
    "$" + parseFloat(pendiente).toFixed(2);
...
async function registrarPagoApartado(event) {
  ...
  const observacion = document.getElementById(
    "pago-apartado-observacion",
  ).value;

  try {
    const response = await fetch(
      `${API_BASE_URL}/api/apartados/${apartadoId}/pago`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
...
async function completarApartado(id) {
  if (
    !confirm("¿Está seguro de completar este apartado? Se generará una venta.")
  )
    return;

  try {
    const response = await fetch(
      `${API_BASE_URL}/api/apartados/${id}/completar`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      },
...
async function cancelarApartado(id) {
  if (
    !confirm(
      "¿Está seguro de cancelar este apartado? Los productos volverán al inventario. El reembolso debe gestionarse manualmente.",
    )
  )
    return;
  ...
  try {
    const response = await fetch(
      `${API_BASE_URL}/api/apartados/${id}/cancelar`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      },
...
async function deleteApartado(id) {
  if (
    !confirm(
      "¿Está seguro de eliminar este apartado permanentemente? Esta acción no se puede deshacer.",
    )
  )
    return;
...
async function loadInventario() {
  ...
  tbody.innerHTML =
    '<tr><td colspan="7" class="loading active"><div class="spinner"></div>Cargando inventario...</td></tr>';
  ...
  try {
    ...
    tbody.innerHTML =
      '<tr><td colspan="7" style="text-align: center; padding: 40px; color: var(--danger);">Error al cargar</td></tr>';
  }
}
  ...
  try {
    ...
    productoSelect.innerHTML =
      ...
      productos
        .map(
          (p) =>
            `<option value="${p.id}">${p.nombre} (Stock: ${p.cantidad_disponible})</option>`,
        )
        .join("");
  } catch (error) {
    console.error("Error loading productos:", error);
...
async function loadConsultas() {
  ...
  try {
    ...
    const response = await fetch(
      `${API_BASE_URL}/api/consultas/ventas?${params.toString()}`,
    );
    const ventas = await response.json();
    ...
    if (ventas.length === 0) {
      tbody.innerHTML =
        '<tr><td colspan="6" style="text-align: center;">No se encontraron ventas</td></tr>';
      return;
    }
...
async function exportarConsultasPDF() {
  ...
  window.open(
    `${API_BASE_URL}/api/consultas/ventas/pdf?${params.toString()}`,
    "_blank",
  );
}
...
async function loadCotizacion() {
  ...
  tbody.innerHTML =
    '<tr><td colspan="3" class="loading active"><div class="spinner"></div>Cargando historial...</td></tr>';
  ...
  try {
    ...
    if (historyData.length === 0) {
      tbody.innerHTML =
        '<tr><td colspan="3" style="text-align: center; padding: 20px;">No hay historial disponible</td></tr>';
      return;
    }
...
async function loadDashboardStats() {
  try {
    ...
    if (document.getElementById("dash-count-empleados"))
      document.getElementById("dash-count-empleados").textContent =
        data.total_empleados || 0;

    if (document.getElementById("dash-count-proveedores"))
      document.getElementById("dash-count-proveedores").textContent =
        data.total_proveedores || 0;

    if (document.getElementById("dash-count-compras"))
      document.getElementById("dash-count-compras").textContent =
        data.total_compras || 0;

    if (document.getElementById("dash-count-productos"))
      document.getElementById("dash-count-productos").textContent =
        data.total_productos || 0;

    if (document.getElementById("dash-count-clientes"))
      document.getElementById("dash-count-clientes").textContent =
        data.total_clientes || 0;

    if (document.getElementById("dash-count-ventas"))
      document.getElementById("dash-count-ventas").textContent =
        data.total_ventas || 0;

    if (document.getElementById("dash-count-apartados"))
      document.getElementById("dash-count-apartados").textContent =
        data.total_apartados_activos || 0;

    if (document.getElementById("dash-count-inventario"))
      document.getElementById("dash-count-inventario").textContent =
        data.total_inventario || 0;

    if (document.getElementById("dash-count-reembolsos"))
      document.getElementById("dash-count-reembolsos").textContent =
        data.total_reembolsos || 0;

    if (document.getElementById("dash-count-cotizacion"))
      document.getElementById("dash-count-cotizacion").textContent = (
...
async function loadReembolsos() {
  ...
  try {
    ...
    if (reembolsos.length === 0) {
      tbody.innerHTML =
        '<tr><td colspan="9" style="text-align: center; padding: 40px; color: #6b7280;">No hay reembolsos registrados</td></tr>';
      return;
    }
    ...
    console.error("Error:", error);
    tbody.innerHTML =
      '<tr><td colspan="9" style="text-align: center; padding: 40px; color: red;">Error al cargar reembolsos</td></tr>';
  }
}
...
async function buscarVentaParaReembolso() {
  ...
  try {
    ...
    if (response.ok) {
      ...
      tasaHistoricaVentaReembolso =
        parseFloat(venta.cotizacion_dolar_bolivares) || 0;
      ...
      document.getElementById("venta-info-reembolso").className =
...
function updateProfileDropdownInfo() {
  ...
  if (avatarLarge) {
    if (currentUser.foto_url) {
      ...
      avatarLarge.innerHTML = currentUser.nombre
...
async function loadProfileDataToForm() {
  ...
  try {
    ...
    if (usuario) {
      ...
      document.getElementById("profile-apellidos").value =
        usuario.apellidos || "";
      document.getElementById("profile-cedula").value = usuario.cedula || "";
      document.getElementById("profile-direccion").value =
        usuario.direccion || "";
...
async function saveProfileChanges(event) {
  ...
  if (!currentUser || !currentUser.id) {
    alert(
      "Error: No hay usuario activo. Por favor, cierra sesión e inicia de nuevo.",
    );
    return;
  }
  ...
  try {
    ...
    if (fotoFile) {
      ...
      const uploadResponse = await fetch(
        `${API_BASE_URL}/api/usuarios/${currentUser.id}/foto`,
        {
          method: "POST",
          body: formData,
        },
...
async function loadEstadisticas() {
  ...
  try {
    ...
    document.getElementById("stat-ventas-hoy").textContent =
      `$${kpis.ventas_hoy_monto.toFixed(2)}`;
    document.getElementById("stat-compras-hoy").textContent =
      `$${kpis.compras_hoy_monto.toFixed(2)}`;
    document.getElementById("stat-beneficio-hoy").textContent =
      `$${kpis.beneficio_hoy.toFixed(2)}`;
    document.getElementById("stat-apartados-activos").textContent =
      kpis.apartados_activos;
  } catch (error) {
  ...
  try {
    ...
    const ctxCategorias = document
      .getElementById("chart-categorias")
      .getContext("2d");
    ...
    const ctxTop = document
      .getElementById("chart-top-productos")
      .getContext("2d");
