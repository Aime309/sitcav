...
function loadDashboard() {
    ...
    if (currentUser.foto_url) {
        const fullUrl = currentUser.foto_url.startsWith('http') ? currentUser.foto_url : `.${currentUser.foto_url}`;
...
async function loadDashboardStats() {
    try {
        ...
        const stockBajo = await fetch(`./api/productos/stock-bajo`).then(r => r.json());
...
function renderCarousel() {
    ...
    productsData.forEach(product => {
        ...
        const imgUrl = product.imagen_url ?
            (product.imagen_url.startsWith('http') ? product.imagen_url : `.${product.imagen_url}`) :
            '';
        card.innerHTML = `
            <img src="${imgUrl}"
                 alt="${product.nombre}"
...
async function openProductModal(productId = null) {
    ...
    // Load categories
    const categories = await fetch(`./api/categorias`).then(r => r.json());
...
async function verFactura(ventaId) {
    window.open(`./api/factura/${ventaId}`, '_blank');
}
...
async function checkUserForRecovery() {
    ...
    try {
        const response = await fetch(`./security-questions/${cedula}`);
        const data = await response.json();
...
function downloadCompraPDF(id) {
    window.open(`./api/compras/${id}/pdf`, '_blank');
}
...
async function checkUserForRecovery() {
    ...
    try {
        const response = await fetch(`./check-user-recovery`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
async function resetPassword() {
    ...
    try {
        const response = await fetch(`./reset-password-recovery`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                user_id: recoveryUserId,
...
async function loadApartados() {
    ...
    try {
        ...
        const url = estadoFilter ? `./api/apartados?estado=${estadoFilter}` : `./api/apartados`;
...
async function registrarPagoApartado(event) {
    ...
    try {
        const response = await fetch(`./api/apartados/${apartadoId}/pago`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
async function completarApartado(id) {
    ...
    try {
        const response = await fetch(`./api/apartados/${id}/completar`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' }
        });
...
async function cancelarApartado(id) {
    ...
    try {
        const response = await fetch(`./api/apartados/${id}/cancelar`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' }
        });
...
async function verDetalleApartado(id) {
    try {
        const response = await fetch(`./api/apartados/${id}`);
        const data = await response.json();
...
function viewApartadoPDF(id) {
    window.open(`./api/apartados/${id}/pdf`, '_blank');
}
...
async function loadConsultas() {
    ...
    try {
        ...
        const response = await fetch(`./api/consultas/ventas?${params.toString()}`);
        const ventas = await response.json();
...
async function exportarConsultasPDF() {
    ...
    window.open(`./api/consultas/ventas/pdf?${params.toString()}`, '_blank');
}
...
async function loadCotizacion() {
    ...
    try {
        // Cargar tasa actual
        const actualRes = await fetch(`./api/cotizacion/actual`);
        ...
        // Cargar historial
        const historyRes = await fetch(`./api/cotizacion`);
...
function printReembolso(id) {
    window.open(`./api/reembolsos/${id}/pdf`, '_blank');
}
...
function updateProfilePhotoPreview(photoUrl, nombre) {
    ...
    if (photoUrl) {
        const fullUrl = photoUrl.startsWith('http') ? photoUrl : `.${photoUrl}`;
...
async function saveProfileChanges(event) {
    ...
    try {
        ...
        if (fotoFile) {
            ...
            const uploadResponse = await fetch(`./api/usuarios/${currentUser.id}/foto`, {
                method: 'POST',
                body: formData
            });
        ...
        console.log('Updating profile with PUT to:', `./api/usuarios/${currentUser.id}`);
...
function updateUserAvatarUI(nombre, fotoUrl) {
    ...
    if (fotoUrl) {
        const fullUrl = fotoUrl.startsWith('http') ? fotoUrl : `.${fotoUrl}`;
...
async function saveNewPassword(event) {
    ...
    try {
        ...
        // Update the password
        const updateResponse = await fetch(`./api/usuarios/${currentUser.id}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
...
async function loadEstadisticas() {
    // Cargar Resumen (KPIs)
    try {
        const responseKpi = await fetch(`./api/estadisticas/resumen`);
    ...
    // Cargar Gráficas
    try {
        const responseHist = await fetch(`./api/estadisticas/historico`);
