...
def generar_factura_pdf(venta_data, negocio_data, cotizacion_bs):
    ...
    elements.append(
        Paragraph(
            f"RIF: {negocio_data['rif']} | Tel: {negocio_data['telefono']}",
            subtitle_style,
        )
    )
    ...
    fecha_venta = datetime.strptime(
        venta_data["fecha_creacion"], "%Y-%m-%d %H:%M:%S.%f"
    )
    direccion_cliente = venta_data["cliente"].get(
    ...
    info_factura = [
        ...
        [
            "CLIENTE",
            f"{venta_data['cliente']['nombre']} {venta_data['cliente']['apellidos']}",
        ],
    ...
    table_productos = Table(
        data_productos,
        colWidths=[0.8 * inch, 3.1 * inch, 1.8 * inch, 1.8 * inch],
    )
    ...
    table_totales = Table(
        data_totales, colWidths=[0.8 * inch, 3.1 * inch, 1.8 * inch, 1.8 * inch]
    )
    ...
    elements.append(
        Paragraph(
            f"Factura generada el {datetime.now().strftime('%d/%m/%Y %H:%M')}",
            footer_style,
        )
    )
...
def generar_reporte_ventas_pdf(ventas_data, fecha_desde=None, fecha_hasta=None):
    ...
    for venta in ventas_data:
        ...
        for detalle in venta["detalles"]:
            total_venta += (
                Decimal(str(detalle["precio_unitario_tipo_dolares"]))
                * detalle["cantidad"]
            )
    ...
    table_ventas = Table(
        data_ventas, colWidths=[1 * inch, 1.5 * inch, 3 * inch, 1.5 * inch]
    )
    table_ventas.setStyle(
        TableStyle(
            [
                ("BACKGROUND", (0, 0), (-1, 0), colors.HexColor("#007bff")),
                ("TEXTCOLOR", (0, 0), (-1, 0), colors.whitesmoke),
...
def generar_factura_compra_pdf(compra_data, negocio_data):
    ...
    proveedor_direccion = compra_data.get("proveedor", {}).get(
        "direccion", "Dirección no registrada"
    )
    ...
    for detalle in compra_data["detalles"]:
        ...
        producto_nombre = detalle.get("producto", {}).get(
            "nombre", "Producto ID " + str(detalle["id_producto"])
        )
...
def generar_reporte_consultas_pdf(ventas_data, filtros_texto):
    ...
    table_ventas = Table(
        data_ventas,
        colWidths=[0.8 * inch, 1.2 * inch, 1.5 * inch, 1.5 * inch, 1.5 * inch],
    )
    table_ventas.setStyle(
        TableStyle(
            [
